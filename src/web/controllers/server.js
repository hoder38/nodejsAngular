...
function getFeedback(item, callback, user) {
    ...
    handleTag(filePath, {time: item.time, height: item.height}, item.name, '', item.status, function(err, mediaType, mediaTag, DBdata) {
        if (err) {
            util.handleError(err, callback, callback);
        }
        var temp_tag = [];
...
app.get('/api/stock/init', function(req, res,next) {
    checkLogin(req, res, next, function(req, res, next) {
        ...
        console.log(req.url);
        console.log(req.body);
        /*api.getTwseXml(8213, 2014, 3, '/home/pi/test1.xml', function(err, xmlPath) {
            if (err) {
                util.handleError(err, next, res);
            }
            res.json({apiOK: true});
        });*/
        stockTool.initXml('/home/pi/tw-gaap-ci-8213-2012Q1-cr.xml', function (err, xml) {
            if (err) {
                util.handleError(err, next, res);
            }
            ...
            stockTool.initXml('/home/pi/tw-gaap-ci-8213-2012Q2-cr.xml', function (err, xml) {
                if (err) {
                    util.handleError(err, next, res);
                }
                ...
                stockTool.initXml('/home/pi/tw-gaap-ci-8213-2012Q3-cr.xml', function (err, xml) {
                    if (err) {
                        util.handleError(err, next, res);
                    }
                    cash = stockTool.getCashflow(xml, cash);
                    asset = stockTool.getAsset(xml, asset);
                    sales = stockTool.getSales(xml, sales);
                    stockTool.initXml('/home/pi/tw-gaap-ci-8213-2012Q4-cr.xml', function (err, xml) {
                        if (err) {
                            util.handleError(err, next, res);
                        }
                        cash = stockTool.getCashflow(xml, cash);
